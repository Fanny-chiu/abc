...
# 設置中文字型
font_path = "font.otf"  # 請確保 font.otf 位於程式同一目錄，或提供正確的絕對路徑
if not os.path.exists(font_path):
    st.error(f"字型檔案 {font_path} 不存在，請確認路徑！")
else:
    font = FontProperties(fname=font_path, size=12)
    plt.rcParams['font.family'] = font.get_name()  # 設置全局字型
...
# 添加標籤和圖例，使用自訂字型
ax.text(-1.4, 1, f'介質1 (n1 = {n1})', fontsize=12, fontproperties=font)
ax.text(-1.4, -1, f'介質2 (n2 = {n2})', fontsize=12, fontproperties=font)
ax.legend(prop=font)
